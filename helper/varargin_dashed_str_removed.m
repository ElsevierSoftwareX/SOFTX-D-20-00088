...
% Helper function to remove the given dashed string (and its datas) in the
% given varargin input.
function in = varargin_dashed_str_removed(str_wo_dash, in, FirstOnly),
    ...
    for ii = 1:numel(ind_str),
        inds = ind_extra_begin(ind_str(ii)):ind_extra_end(ind_str(ii));
        B_in(inds) = false;
    end
    in = in(B_in);
end
