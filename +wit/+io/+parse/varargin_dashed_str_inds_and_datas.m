...
function [inds, datas, in_wo] = varargin_dashed_str_inds_and_datas(str_wo_dash, in, N),
    if nargin < 3, [~, ind_dashed_begin, ind_dashed_end] = wit.io.parse.varargin_dashed_str(str_wo_dash, in);
    else, [~, ind_dashed_begin, ind_dashed_end] = wit.io.parse.varargin_dashed_str(str_wo_dash, in, N); end
