...
% Simple examples of (A1 i-iv.) file importing to get started. This calls wit.io.wip.read
...
h = wit.io.misc.msgbox({'{\bf\fontsize{12}\color{magenta}EXAMPLE CASE 1 A:}' ...
...
%-------------------------------------------------------------------------%
wit.io.misc.msgbox({'{\bf\fontsize{12}{\color{magenta}(A1 i.)} Load and browse file contents in a GUI by executing:}' ...
    '' ...
    '{\bf\fontname{Courier}[O\_wid, O\_wip, O\_wid\_HtmlNames] = wit.io.wip.read(file);}' ...
    '' ...
...
[O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file); % (1A) Browse the file contents

% '-Manager'-option with single-dashed options require enclosing by
% {}-brackets:
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-Manager', {'-all'}); % To show also the non-plottable data

% The single-dashed options can be replaced by double-dashed options and
% passed on via '-Manager'-option without {}-brackets:
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-Manager', '--all'); % To show also the non-plottable data
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-Manager', '--Type', 'TDGraph'); % To show only TDGraphs
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-Manager', '--Type', 'TDGraph', '--SubType', 'Image'); % To show only Image<TDGraphs
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-Manager', '--Type', 'TDBitmap', 'TDGraph', 'TDImage', '--SubType', '', 'Image', ''); % To show only TDBitmap, Image<TDGraphs and TDImage
...
%-------------------------------------------------------------------------%
h = wit.io.misc.msgbox({'{\bf\fontsize{12}{\color{magenta}(A1 ii.)} OR, load all file contents without GUI by executing:}' ...
    '' ...
    '{\bf\fontname{Courier}[...] = wit.io.wip.read(file, ''-all'');}' ...
    '' ...
    '{\bf\fontsize{12}{\color{magenta}(A1 iii.)} OR, specify ''-ifall'' to ask whether to browse or load the content:}' ...
    '' ...
    '{\bf\fontname{Courier}[...] = wit.io.wip.read(file, ''-ifall'');}' ...
    '' ...
    '{\bf\fontsize{12}{\color{magenta}(A1 iv.)} OR, load by browsing file system for WITec Project/Data (*.wip/*.wid) -files by executing:}' ...
    '' ...
    '{\bf\fontname{Courier}[...] = wit.io.wip.read();}' ...
    '' ...
...
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-all'); % (1B) Load all file content
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(file, '-ifall'); % (1C) Ask whether to browse or load the file content
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.wip.read(); % (1D) Browse the file system
%-------------------------------------------------------------------------%
