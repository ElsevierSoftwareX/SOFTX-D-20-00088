...
wit.io.misc.msgbox({'{\bf\fontsize{12}{\color{magenta}(A1 i.)} Load and browse file contents in a GUI by executing:}' ...
    '' ...
    '{\bf\fontname{Courier}[O\_wid, O\_wip, O\_wid\_HtmlNames] = wit.io.read(file);}' ...
    '' ...
...
[O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file); % (1A) Browse the file contents

% '-Manager'-option with single-dashed options require enclosing by
% {}-brackets:
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-Manager', {'-all'}); % To show also the non-plottable data

% The single-dashed options can be replaced by double-dashed options and
% passed on via '-Manager'-option without {}-brackets:
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-Manager', '--all'); % To show also the non-plottable data
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-Manager', '--Type', 'TDGraph'); % To show only TDGraphs
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-Manager', '--Type', 'TDGraph', '--SubType', 'Image'); % To show only Image<TDGraphs
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-Manager', '--Type', 'TDBitmap', 'TDGraph', 'TDImage', '--SubType', '', 'Image', ''); % To show only TDBitmap, Image<TDGraphs and TDImage
...
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-all'); % (1B) Load all file content
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-ifall'); % (1C) Ask whether to browse or load the file content
% [O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(); % (1D) Browse the file system
%-------------------------------------------------------------------------%
