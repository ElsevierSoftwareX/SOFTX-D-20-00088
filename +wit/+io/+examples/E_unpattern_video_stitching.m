...
h = wit.io.misc.msgbox({'{\bf\fontsize{12}\color{magenta}EXAMPLE CASE E:}' ...
'{\bf\fontsize{12}UNPATTERN VIDEO STITCHING ' ...
'IMAGES}' ...
'' ...
'\bullet If unfamiliar with ''wit\_io'', then go through the previous examples first.'}, '-TextWrapping', false);
...
[O_wid, O_wip, O_wid_HtmlNames] = wit.io.read(file, '-all', '-Manager', '--Type', 'TDBitmap', 'TDText');
...
%-------------------------------------------------------------------------%
h = wit.io.misc.msgbox({'{\bf\fontsize{12}{\color{magenta}(E)} Unpatterning the Video Stitching ' ...
'images:}' ...
'' ...
