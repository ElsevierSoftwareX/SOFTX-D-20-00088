...
function [O_wid, O_wip, O_wid_HtmlNames] = read(varargin),
    ...
    ind_extra_begin = wit.io.parse.varargin_dashed_str_inds('', varargin);
    if isempty(ind_extra_begin), files = varargin;
    else, files = varargin(1:ind_extra_begin(1)-1); end
    
    showProjectManager = ~wit.io.parse.varargin_dashed_str_exists('all', varargin); % By default, show Project Manager
    show_ui_ifall = wit.io.parse.varargin_dashed_str_exists('ifall', varargin);
    
    [exists, datas] = wit.io.parse.varargin_dashed_str_exists_and_datas('LimitedRead', varargin, -1);
    ...
    Params = wit.io.parse.varargin_dashed_str_datas('Params', varargin);
    ...
    datas = wit.io.parse.varargin_dashed_str_datas('DataUnit', varargin, -1);
    if numel(datas) > 0, O_wip.ForceDataUnit = datas{1}; end
    
    datas = wit.io.parse.varargin_dashed_str_datas('SpectralUnit', varargin, -1);
    if numel(datas) > 0, O_wip.ForceSpectralUnit = datas{1}; end
    
    datas = wit.io.parse.varargin_dashed_str_datas('SpaceUnit', varargin, -1);
    if numel(datas) > 0, O_wip.ForceSpaceUnit = datas{1}; end
    
    datas = wit.io.parse.varargin_dashed_str_datas('TimeUnit', varargin, -1);
    if numel(datas) > 0, O_wip.ForceTimeUnit = datas{1}; end
    
    datas = wit.io.parse.varargin_dashed_str_datas('Manager', varargin);
    ManagerVarargin = {};
