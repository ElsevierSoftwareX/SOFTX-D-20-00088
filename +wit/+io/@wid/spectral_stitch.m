...
function [new_obj, Graph, Data, W, D] = spectral_stitch(obj, varargin),
    ...
    isdebug = wit.io.parse.varargin_dashed_str_exists('debug', varargin);
    
    % Parse the input and keep only TDGraph wid objects
    bw_valid_wid = cellfun(@(x) isa(x, 'wit.io.wid'), varargin); % Test if wid
    ...
    [Graph, Data, W, D] = wit.io.wid.spectral_stitch_helper(Graphs_nm, Datas, isdebug);
    ...
    if AutoCreateObj,
        ...
        new_SpectralT = wit.io.wid.new_Transformation_LUT(obj.Tag.Root, numel(Graph)); % This does not add newly created object to Project yet!
        ...
        new_SpectralT_Data.TDLUTTransformation.LUTSize = numel(Graph); % Ignored by wit_io, but used in WITec software
