...
function write(obj, File) % For saving WIT-formatted WID-files!
    if nargin < 2 || iscell(File), % If no or many filenames specified
        for ii = 1:numel(obj),
            ...
            O_wit = wit.io.wid.new(wit.io.wip.get_Root_Version(obj(ii))); % Create minimal data for each object
    ...
    elseif ischar(File), % If only one filename specified for all, then save all to the same
        ...
        if numel(obj) > 0, O_wit = wit.io.wid.new(Version);
        else, O_wit = wit.io.wid.new(); end % Create minimal data for all objects
