...
function [obj, N_bests, Datas] = unpattern_video_stitching(obj, varargin),
    ...
    TDText = wit.io.parse.varargin_dashed_str_datas('TDText', varargin);
    TDText = TDText(cellfun(@(x) isa(x, 'wit.io.wid'), TDText)); % Keep only wid
    ...
    for ii = 1:numel(obj),
        ...
        [Datas{ii}, N_bests(ii), cropIndices] = wit.io.wid.unpattern_video_stitching_helper(obj(ii).Data, [N_X N_Y], varargin{:});
