...
function [new_obj, image_mask] = image_mask_editor(obj, image_mask),
    ...
    if AutoCreateObj,
        new_obj = wit.io.wid.new_Image(obj.Tag.Root); % This does not add newly created object to Project yet!
            ...
            if masking_mode == 1, % Freehand-mode
                if isempty(old_CP),
                    ...
                    if isMouseUp,
                        ...
                        L = wit.io.lib.label.label(image_mask);
                        stats = wit.io.fun.myregionprops(L);
                        PILS = cellfun(@(pil) pil(1), {stats.PixelIdxList});
                        [XS, YS] = ind2sub(size(image_mask), PILS);
                        in = wit.io.fun.myinpolygon(XS, YS, old_CP(:,1), old_CP(:,2));
                        for ii = 1:numel(in),
