...
function F = fun_lineshape_voigtian(P, X),
    ...
    nominator = real(wit.io.lib.fadf.fadf(Z)); % Voigtian lineshape profile
    divisor = real(wit.io.lib.fadf.fadf(z)); % Maximum of the Voigtian lineshape profile % = exp(imag(z_max).^2).*erfc(imag(z_max));
...
%     nominator_2 = real(-wit.io.lib.fadf.fadf(-Z)); % Shoots quickly to infinity when Z > 26
%     
...
%     % 
%     % divisor = real(wit.io.lib.fadf.fadf(z)) ...
...
%     ratio = bsxfun(@plus, 1, nominator_2./nominator_1) .* subratio;
    ...
    if any(B_L), F(:,B_L) = wit.io.fun.fit.fun_lineshape_lorentzian(P(1:4,B_L), X(:,B_L)); end % Backward compatible with R2011a
    if any(B_G), F(:,B_G) = wit.io.fun.fit.fun_lineshape_gaussian(P([1:2 5 4],B_G), X(:,B_G)); end % Backward compatible with R2011a
end
