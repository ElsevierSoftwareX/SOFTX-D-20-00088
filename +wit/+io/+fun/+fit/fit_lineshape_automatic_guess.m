...
function P0 = fit_lineshape_automatic_guess(x, Y, dim),
    ...
    A = wit.io.fun.mtrapz(x, Y, 1); % Robust parameter but overestimates, especially when the lineshape is fully within range.
    ...
    Pos = wit.io.fun.mtrapz(x, bsxfun(@times, x, Y), 1) ./ A; % Robust parameter
    ...
    antiline_length = wit.io.fun.fit.bw2lines(~B_peaks); % Convert B to lines
    B_peaks(antiline_length >= 1 & antiline_length <= 2) = true; % Fill in (reduce effect of noise)
    
    % Second: FIND and KEEP best-length lines
    % Determine B lengths per spectrum or 2nd dimension
    line_length = wit.io.fun.fit.bw2lines(B_peaks); % Convert B to lines
