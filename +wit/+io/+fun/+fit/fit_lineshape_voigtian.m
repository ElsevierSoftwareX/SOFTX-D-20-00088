...
function [P, R2, SSres, Y_fit, R2_total, SSres_total] = fit_lineshape_voigtian(x, Y, P0, dim, varargin),
    ...
    datas = wit.io.parse.varargin_dashed_str_datas('Fwhm_G', varargin, -1);
    if numel(datas) > 0,
        Fwhm_G = datas{1};
        ...
        varargin = wit.io.parse.varargin_dashed_str_removed('Fwhm_G', varargin); % Remove this option from varargin!
    ...
    [P, R2, SSres, Y_fit, R2_total, SSres_total] = wit.io.fun.fit.fit_lineshape_arbitrary(@wit.io.fun.fit.fun_lineshape_voigtian, x, Y, P0, dim, varargin{:});
end
