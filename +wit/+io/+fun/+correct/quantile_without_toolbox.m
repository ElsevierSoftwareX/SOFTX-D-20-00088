...
% Used by wit.io.fun.correct.nanmedian_without_toolbox
function Y = quantile_without_toolbox(X, p, dim),
    ...
    X = wit.io.fun.dim_first_permute(double(X), dim); % Minor bottleneck
    S = size(X);
...
%     Y(bw_nz) = X(r_prev) + (0.5+r).*(X(r_next)-X(r_prev)); % Alternative
    ...
    % Restore y shape
    Y = wit.io.fun.dim_first_ipermute(reshape(Y, [1 S(2:end)]), dim);
end
