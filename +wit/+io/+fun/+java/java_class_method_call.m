...
% [N_read, buffer] = wit.io.fun.java.java_class_method_call('java.io.InputStream', 'read', jFIS, buffer, int32(0), int32(numel(buffer)); % This indirect call updates buffer but direct call to N_read = jFIS.read(buffer, 0, numel(buffer)); will not.
function varargout = java_class_method_call(classname, methodname, jObj, varargin),
    ...
    [jObjects, jClasses] = wit.io.fun.java.java_objects_from_varargin(varargin{:});
