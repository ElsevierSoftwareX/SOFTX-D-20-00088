...
% (4) fun(data, dim, ...) = wit.io.fun.clever_statistics_and_outliers (by default):
...
function varargout = rolling_window_analysis(X, W, isCircular, fun, varargin),
    % Parse input
    if nargin < 4, fun = @wit.io.fun.clever_statistics_and_outliers; end % Default fun
    if nargin < 3, isCircular = false; end
    if nargin < 2, W = 0; end
    
    % Create a rolling window analysis dataset
    X_RWA = wit.io.fun.rolling_window_analysis_transformation(X, W, isCircular); % Also tests the input W and isCircular
