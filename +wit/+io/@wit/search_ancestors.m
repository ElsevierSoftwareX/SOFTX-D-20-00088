...
function tags = search_ancestors(obj, varargin),
    ...
    if ~any(match), tags = wit.io.wit.empty; return; end % Return empty if no matches
    if numel(varargin) == 1, tags = obj(match); % Return matches if no more criteria found
    else, tags = search_ancestors([obj(match).Parent wit.io.wit.empty], varargin{2:end}); end % Continue search with the matched obj children
end
