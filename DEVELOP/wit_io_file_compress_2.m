...
function wit_io_file_compress_2(file, files, datas, varargin),
    ...
    persistent zst_library zip_library;
    
    % Use ZIP compression
    if strcmpi('.zip', Compressor),
        MaxBlockSize = 1024.^2; % By default, 1 MB max subblocksize per write
        compressor_multiple_files = true;
        try, % Try use zip4j for its superior speed
            % Documentation: https://javadoc.io/doc/net.lingala.zip4j/zip4j/latest/index.html
            if isempty(zip_library), % Load java library only once per session
                compressor_library = {'wit_io_file_compress_2', 'zip4j', 'zip4j-2.5.2.jar'};
                zip_library = fullfile(fileparts(mfilename('fullpath')), compressor_library{:});
                javaaddpath(zip_library);

                % Test if successful OR error!
                import net.lingala.zip4j.io.outputstream.ZipOutputStream;
            end
            compressor_constructor = @compress_zip4j_construct;
            compressor = @compress_zip4j;
            compressor_finish = @compress_zip4j_finish;
        catch,
...
%         ocuo{1} = onCleanup(@() jco.close()); % Ensure safe closing of the compressed file in the end
        ...
        jfo = java.io.FileOutputStream(jf);
        ocuo{2} = onCleanup(@() jfo.close()); % Ensure safe closing of the compressed file in the end
        
        jco = org.apache.tools.zip.ZipOutputStream(jfo);
        ocuo{1} = onCleanup(@() jco.close()); % Ensure safe closing of the compressed file in the end
        
%         jco.setEncoding('UTF-8'); % Required for older MATLAB versions
...
%         jco.finish();
    ...
    function compress_zip4j_construct(),
        jf = java.io.File(file);
        ...
        jco = net.lingala.zip4j.io.outputstream.ZipOutputStream(jbo);
        ocuo{1} = onCleanup(@() jco.close()); % Ensure safe closing of the compressed file in the end
    end
    function compress_zip4j_finish(),
        
    end
    function compress_zip4j(),
        % Compress the given files and datas in loop
        for ii = 1:numel(files),
            entry_file = files{ii};
            entry_data = typecast(datas{ii}, 'int8'); % Java-friendly int8
            ...
            % Create a file entry
            entry = net.lingala.zip4j.model.ZipParameters();
            entry.setFileNameInZip(entry_file);
            entry.setEntrySize(numel(entry_data)); % Set the uncompressed size of the entry data (i.e. to allow 64-bit ZIP if needed)
            
            % Set compression level
            if ~isempty(CompressionLevel), % If empty, then use built-in default
                if CompressionLevel == 1,
                    CL = net.lingala.zip4j.model.enums.CompressionLevel.FASTEST;
                elseif CompressionLevel == 3,
                    CL = net.lingala.zip4j.model.enums.CompressionLevel.FAST;
                elseif CompressionLevel == 5,
                    CL = net.lingala.zip4j.model.enums.CompressionLevel.NORMAL;
                elseif CompressionLevel == 7,
                    CL = net.lingala.zip4j.model.enums.CompressionLevel.MAXIMUM;
                elseif CompressionLevel == 9,
                    CL = net.lingala.zip4j.model.enums.CompressionLevel.ULTRA;
                else,
                    CL = net.lingala.zip4j.model.enums.CompressionLevel(CompressionLevel);
                end
                entry.setCompressionLevel(CL);
            end
