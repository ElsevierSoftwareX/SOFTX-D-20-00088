...
function [obj, N_bests, Datas] = unpattern_video_stitching(obj, varargin),
    ...
    TDText = TDText(cellfun(@(x) isa(x, 'wid'), TDText)); % Keep only wid
    ...
    for ii = 1:numel(obj),
        ...
        [Datas{ii}, N_bests(ii), cropIndices] = wid.unpattern_video_stitching_helper(obj(ii).Data, [N_X N_Y], varargin{:});
