...
function wid_Data_set_Bitmap(obj, in),
    ...
    elseif Version == 5, % WITec Project 2.x
        ...
        out = zeros(54+numel(in), 1, 'uint8'); % Initialize all to zero
        out(1:2) = 'BM';
...
%         out(9:10) = typecast(uint16(0), 'uint8'); % Reserved; actual value depends on the application that creates the image
        ...
        out(19:22) = typecast(int32(size(in, 2)), 'uint8'); % The bitmap width in pixels (signed integer)
        out(23:26) = typecast(int32(size(in, 3)), 'uint8'); % The bitmap height in pixels (signed integer)
        out(27:28) = typecast(uint16(1), 'uint8'); % The number of color planes (must be 1)
        out(29:30) = typecast(uint16(8.*size(in, 1)), 'uint8'); % The number of bits per pixel, which is the color depth of the image. Typical values are 1, 4, 8, 16, 24 and 32.
%         out(31:34) = typecast(uint32(0), 'uint8'); % The compression method being used. See the next table for a list of possible values
        out(35:38) = typecast(uint32(numel(in)), 'uint8'); % The image size. This is the size of the raw bitmap data; a dummy 0 can be given for BI_RGB bitmaps.
