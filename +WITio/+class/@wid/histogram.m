...
%         Data_upper_quantile = interp1([0 1], [cmin cmax], upper_quantile, 'linear'); % Approximate upper quantile
        ...
        Bin_Centers = reshape((Bin_Edges(1:end-1)+Bin_Edges(2:end))./2, [], 1); % Get Bin Centers because WITio.class.wid.plot uses built-in bar-function
    ...
    if AutoCreateObj,
        new_obj = WITio.class.wid.new_Graph(obj.Tag.Root); % This does not add newly created object to Project yet!
        ...
        new_TLUT = WITio.class.wid.new_Transformation_LUT(obj.Tag.Root, numel(Bin_Centers)); % This does not add newly created object to Project yet!
        new_TLUT_Data = new_TLUT.Data; % Get formatted struct once to speed-up
        new_TLUT_Data.TDLUTTransformation.LUT = Bin_Centers;
        new_TLUT_Data.TDLUTTransformation.LUTSize = numel(Bin_Centers); % Ignored by WITio, but used in WITec software
        new_TLUT_Data.TDLUTTransformation.LUTIsIncreasing = true; % Ignored by WITio, but used in WITec software
        new_TLUT.Data = new_TLUT_Data; % Save all changes once

        new_ILUT = WITio.class.wid.new_Interpretation_Z(obj.Tag.Root); % This does not add newly created object to Project yet!
