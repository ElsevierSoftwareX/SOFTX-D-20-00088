...
function [new_obj, image_mask] = image_mask_editor(obj, image_mask),
    ...
    if ~isempty(obj) || nargin > 1,
        ...
        masking_mode = 0;
        show_mask();
        WITio.ui.sidebar_popup(Fig, 'Masking value:', {'False', 'True'}, @update_value, 1, false, [1 -1 1 1]);
        WITio.ui.sidebar_button(Fig, [], 'Clear mask', @clear, [1 1 1 -1]);
        WITio.ui.sidebar_button(Fig, [], 'Invert mask', @invert, [1 1 1 -1]);
        ...
        WITio.ui.sidebar_popup(Fig, 'Masking tool:', str_Popup, @update, 1, false, [1 -1 1 1]);
    ...
    if AutoCreateObj,
        new_obj = WITio.class.wid.new_Image(obj.Tag.Root); % This does not add newly created object to Project yet!
        ...
        if WITio.class.wip.get_Root_Version(obj) == 7,
            ...
            if masking_mode == 1, % Freehand-mode
                if isempty(old_CP),
                    ...
                    if isMouseUp,
                        ...
                        L = WITio.lib.label.label(image_mask);
