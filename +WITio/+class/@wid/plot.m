...
function h = plot(obj, varargin),
    ...
    showSidebar = ~WITio.parse.varargin_dashed_str_exists('nosidebar', varargin); % By default, show sidebar
    showPreview = ~WITio.parse.varargin_dashed_str_exists('nopreview', varargin); % By default, show preview
    showCursor = ~WITio.parse.varargin_dashed_str_exists('nocursor', varargin); % By default, show cursor
    showPosition = WITio.parse.varargin_dashed_str_datas('position', varargin);
    [showScalebar, vararginScalebar] = WITio.parse.varargin_dashed_str_exists_and_datas('scalebar', varargin);
    obj_compare = WITio.parse.varargin_dashed_str_datas('compare', varargin);
    ...
    % Mask Data
    MaskDatas = WITio.parse.varargin_dashed_str_datas('mask', varargin);
    ...
    if ~isempty(Data),
        switch(obj.Type),
            case 'TDBitmap',
                ...
                if showSidebar && showCursor, WITio.ui.sidebar_cursor([], [], [], @transform_CP); end
            case 'TDGraph',
                ...
                switch(obj.SubType),
                    case 'Image', % Image
                        if showSidebar,
                            ...
                            fun_refresh = WITio.ui.sidebar_clim(Fig);
                            WITio.ui.sidebar_perceptually_uniform_colormaps(Fig);
                            WITio.ui.sidebar_sum_filter(Fig, @updateGraphImage, fun_refresh);
                            if showCursor,
                                if ~showPreview, WITio.ui.sidebar_cursor(Fig, [], [], @transform_CP);
                                else, WITio.ui.sidebar_cursor(Fig, @subPreview, [], @transform_CP); end
                            end
                        else,
                    ...
                    case 'Line', % Line
                        plotSpectrum(1, 1);
                        if showSidebar,
                            if showCursor, WITio.ui.sidebar_cursor(Fig); end
                            WITio.ui.sidebar_index_slider(Fig, Info.XSize, @(Ind) plotSpectrum(Ind, 1));
                            fun_auto = WITio.ui.sidebar_checkbox(Fig, 'Autoscale', 1);
                        end
                    case 'Point', % Point
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.ui.sidebar_cursor(Fig); end
                    case 'Array', % Array
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.ui.sidebar_cursor(Fig); end
                    ...
                    case 'Mask', % Mask
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.ui.sidebar_cursor(Fig); end        % CUSTOM TYPE
                    case 'Volume', % Volume (CUSTOM)
                        if showSidebar,
                            updateGraphVolume([-inf inf]);
                            WITio.ui.sidebar_perceptually_uniform_colormaps(Fig);
                            WITio.ui.sidebar_sum_filter(Fig, @updateGraphVolume);
                        else,
            ...
            case 'TDImage', % Image
                ...
                if showSidebar,
                    WITio.ui.sidebar_clim(Fig);
                    WITio.ui.sidebar_perceptually_uniform_colormaps(Fig);
                    if showCursor, WITio.ui.sidebar_cursor(Fig, [], [], @transform_CP); end
                end
            otherwise, % Text OR (un)formatted DataTree
                ...
                if showSidebar, % Show sidebar if requested
                    h_mainbar = WITio.ui.sidebar(Fig);
    ...
    function Text_cell = DataTree2TextCell(DT),
        ...
        for ll = 1:numel(values),
            ...
            if isstruct(value_ll), % CASE: A struct (== sub DataTree)
                ...
                if isa(value_ll, 'WITio.class.wid'), % CASE: A wid object
...
%             end
    ...
    function updateGraphVolume(filter_range),
        set(0, 'CurrentFigure', Fig);
        Data_range = WITio.class.wid.crop_Graph_with_bg_helper(Data, Info.Graph, filter_range);
    ...
    function subPreview(CP),
        if ~isBusy, % Proceed only if NOT busy
            ...
            if isempty(Fig_sub) || ~ishandle(Fig_sub), 
                ...
                if showSidebar,
                    WITio.ui.sidebar_cursor(Fig_sub);
                    fun_auto = WITio.ui.sidebar_checkbox(Fig_sub, 'Autoscale', 1);
                end
            end
            % Show spectrum data
...
%             colormap(WITio.lib.perceptually_uniform_colormap('inferno')); % Use inferno by default
            colorbar('HitTest', 'off'); % HitTest 'off' added for backward compability of WITio.ui.sidebar_cursor
    ...
    function h = plot_Spectrum(Fig, X, XUnit, Y, YUnit, fun, isAuto),
        ...
        if isempty(XUnit), XUnit = WITio.class.wip.ArbitraryUnit; end
        xlabel(Ax, XUnit);

        if isempty(YUnit), YUnit = WITio.class.wip.ArbitraryUnit; end
        ylabel(Ax, YUnit);
    end
end
