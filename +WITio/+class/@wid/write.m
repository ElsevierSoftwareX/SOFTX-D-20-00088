...
function write(obj, File) % For saving WIT-formatted WID-files!
    if nargin < 2 || iscell(File), % If no or many filenames specified
        for ii = 1:numel(obj),
            ...
            O_wit = WITio.class.wid.new(WITio.class.wip.get_Root_Version(obj(ii))); % Create minimal data for each object
            ...
            O_wit = WITio.class.wip.append(O_wit, unique(O_wits));
            O_wit.write(File{ii});
            delete(O_wit);
        end
    elseif ischar(File), % If only one filename specified for all, then save all to the same
        Version = WITio.class.wip.get_Root_Version(obj(1));
        if numel(obj) > 0, O_wit = WITio.class.wid.new(Version);
        else, O_wit = WITio.class.wid.new(); end % Create minimal data for all objects
        O_wits = WITio.class.wit.empty;
        for ii = 1:numel(obj),
            if WITio.class.wip.get_Root_Version(obj(ii)) ~= Version,
