...
function [I_best, N_best, cropIndices] = unpattern_video_stitching_helper(I, N_SI_XY, varargin),
    ...
    out = WITio.parse.varargin_dashed_str_removed('', varargin);
    N_test = [];
    if numel(out) > 0, N_test = out{1}; end
    
    % Parse varargin input
    RestoreTooBright = WITio.parse.varargin_dashed_str_exists('RestoreTooBright', varargin); % By default, do not restore too bright
    IsFlat = WITio.parse.varargin_dashed_str_exists('Flat', varargin); % By default, do not assume flatness
    AdditiveMode = WITio.parse.varargin_dashed_str_exists('AdditiveMode', varargin); % By default, fix in a multiplicative mode
    UseMedian = WITio.parse.varargin_dashed_str_exists('UseMedian', varargin); % By default, use mean values
    RollingWindowAnalysis = WITio.parse.varargin_dashed_str_exists('RollingWindowAnalysis', varargin); % By default, smoothen the image
    GlobalRollingWindowAnalysis = WITio.parse.varargin_dashed_str_exists('GlobalRollingWindowAnalysis', varargin); % By default, smoothen the image
    CropEdgePatterns = WITio.parse.varargin_dashed_str_exists('Crop', varargin); % By default, do not crop the image
    AutoIgnoreChannels = ~WITio.parse.varargin_dashed_str_exists('DisableAutoIgnoreChannels', varargin); % By default, automatically ignore channels that have 50% too dark or too bright in the image
    NoReferences = ~WITio.parse.varargin_dashed_str_exists('BestContinuity', varargin); % By default, do not use references to improve the pattern-to-pattern continuity
    Debug = WITio.parse.varargin_dashed_str_exists('Debug', varargin); % By default, do not show Debug visuals
    
%     EdgePatterns = ~WITio.parse.varargin_dashed_str_exists('DisableEdgePatterns', varargin); % By default, enable edge patterns
...
% provided with [left, right, top, bottom] input for customization.
    ...
    datas = WITio.parse.varargin_dashed_str_datas('Outliers', varargin, -1);
    B_Outliers = false(W, H);
    if numel(datas) > 0, B_Outliers = datas{1}; end

    % Check if MinSigmasThreshold was specified
    datas = WITio.parse.varargin_dashed_str_datas('MinSigmasThreshold', varargin, -1);
    ...
    datas = WITio.parse.varargin_dashed_str_datas('TrueBlackColor', varargin, -1);
    TrueBlackColor = 0;
    ...
    datas = WITio.parse.varargin_dashed_str_datas('LocalWindowSize', varargin, -1);
    LocalWindowSize = 17;
    if numel(datas) > 0, LocalWindowSize = datas{1}; end
    
    % Check if OverlapRatio was specified
    datas = WITio.parse.varargin_dashed_str_datas('OverlapRatio', varargin, -1);
    ...
    datas = WITio.parse.varargin_dashed_str_datas('LowResMaxPixels', varargin, -1);
    ...
    datas = WITio.parse.varargin_dashed_str_datas('HighResMaxPixels', varargin, -1);
    ...
    datas = WITio.parse.varargin_dashed_str_datas('IgnoreChannels', varargin, -1);
    IgnoreChannels = [];
