...
function [I_best, N_best, cropIndices] = unpattern_video_stitching_helper(I, N_SI_XY, varargin),
    ...
    out = WITio.misc.varargin_dashed_str.removed('', varargin);
    N_test = [];
    if numel(out) > 0, N_test = out{1}; end
    
    % Parse varargin input
    RestoreTooBright = WITio.misc.varargin_dashed_str.exists('RestoreTooBright', varargin); % By default, do not restore too bright
    IsFlat = WITio.misc.varargin_dashed_str.exists('Flat', varargin); % By default, do not assume flatness
    AdditiveMode = WITio.misc.varargin_dashed_str.exists('AdditiveMode', varargin); % By default, fix in a multiplicative mode
    UseMedian = WITio.misc.varargin_dashed_str.exists('UseMedian', varargin); % By default, use mean values
    RollingWindowAnalysis = WITio.misc.varargin_dashed_str.exists('RollingWindowAnalysis', varargin); % By default, smoothen the image
    GlobalRollingWindowAnalysis = WITio.misc.varargin_dashed_str.exists('GlobalRollingWindowAnalysis', varargin); % By default, smoothen the image
    CropEdgePatterns = WITio.misc.varargin_dashed_str.exists('Crop', varargin); % By default, do not crop the image
    AutoIgnoreChannels = ~WITio.misc.varargin_dashed_str.exists('DisableAutoIgnoreChannels', varargin); % By default, automatically ignore channels that have 50% too dark or too bright in the image
    NoReferences = ~WITio.misc.varargin_dashed_str.exists('BestContinuity', varargin); % By default, do not use references to improve the pattern-to-pattern continuity
    Debug = WITio.misc.varargin_dashed_str.exists('Debug', varargin); % By default, do not show Debug visuals
    
%     EdgePatterns = ~WITio.misc.varargin_dashed_str.exists('DisableEdgePatterns', varargin); % By default, enable edge patterns
...
% provided with [left, right, top, bottom] input for customization.
    ...
    datas = WITio.misc.varargin_dashed_str.datas('Outliers', varargin, -1);
    B_Outliers = false(W, H);
    if numel(datas) > 0, B_Outliers = datas{1}; end

    % Check if MinSigmasThreshold was specified
    datas = WITio.misc.varargin_dashed_str.datas('MinSigmasThreshold', varargin, -1);
    ...
    datas = WITio.misc.varargin_dashed_str.datas('TrueBlackColor', varargin, -1);
    TrueBlackColor = 0;
    ...
    datas = WITio.misc.varargin_dashed_str.datas('LocalWindowSize', varargin, -1);
    LocalWindowSize = 17;
    if numel(datas) > 0, LocalWindowSize = datas{1}; end
    
    % Check if OverlapRatio was specified
    datas = WITio.misc.varargin_dashed_str.datas('OverlapRatio', varargin, -1);
    ...
    datas = WITio.misc.varargin_dashed_str.datas('LowResMaxPixels', varargin, -1);
    ...
    datas = WITio.misc.varargin_dashed_str.datas('HighResMaxPixels', varargin, -1);
    ...
    datas = WITio.misc.varargin_dashed_str.datas('IgnoreChannels', varargin, -1);
    IgnoreChannels = [];
