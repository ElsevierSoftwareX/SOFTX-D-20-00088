...
function [ValueUnit, varargout] = interpret(I, Unit_new, Unit_old, varargin),
    ...
    PixelUnit = WITio.class.wip.ArbitraryUnit; % Treated same way as the default units
    ...
    switch(Type),
        case Types{1}, % SpaceInterpretation
            Units = {WITio.class.wip.interpret_StandardUnit('m'), @(x) 1e-6.*x, @(y) 1e6.*y; ... % m
                WITio.class.wip.interpret_StandardUnit('mm'),     @(x) 1e-3.*x, @(y) 1e3.*y; ... % mm
                WITio.class.wip.interpret_StandardUnit('µm'),     @(x) x,       @(y) y; ... % µm % DEFAULT
                WITio.class.wip.interpret_StandardUnit('nm'),     @(x) 1e3.*x,  @(y) 1e-3.*y; ... % nm
                WITio.class.wip.interpret_StandardUnit('Å'),      @(x) 1e4.*x,  @(y) 1e-4.*y; ... % Å
                WITio.class.wip.interpret_StandardUnit('pm'),     @(x) 1e6.*x,  @(y) 1e-6.*y}; % pm
        ...
        case Types{2}, % SpectralInterpretation
            ...
            Units = {WITio.class.wip.interpret_StandardUnit('nm'),   @(x) x,                           @(y) y; ... % nm % DEFAULT
                WITio.class.wip.interpret_StandardUnit('µm'),        @(x) 1e-3.*x,                     @(y) 1e3.*y; ... % µm
                WITio.class.wip.interpret_StandardUnit('1/cm'),      @(x) 1e7./x,                      @(y) 1e7./y; ... % 1/cm
                WITio.class.wip.interpret_StandardUnit('rel. 1/cm'), @(x) 1e7.*(1./x0-1./x),           @(y) 1./(1./x0-1e-7.*y); ... % rel. 1/cm
                WITio.class.wip.interpret_StandardUnit('eV'),        @(x) 1.23984193e3./x,             @(y) 1.23984193e3./y; ... % eV
                WITio.class.wip.interpret_StandardUnit('meV'),       @(x) 1.23984193e6./x,             @(y) 1.23984193e6./y; ... % meV
                WITio.class.wip.interpret_StandardUnit('rel. eV'),   @(x) -1.23984193e3.*(1./x0-1./x), @(y) 1./(1./x0+y./1.23984193e3); ... % rel. eV
                WITio.class.wip.interpret_StandardUnit('rel. meV'),  @(x) -1.23984193e6.*(1./x0-1./x), @(y) 1./(1./x0+y./1.23984193e6)}; % rel. meV
            ValueUnit = Units{1,1}; % Default ValueUnit
        case Types{3}, % TimeInterpretation
            Units = {WITio.class.wip.interpret_StandardUnit('h'), @(x) x./3600, @(y) 3600.*y; ... % h
                WITio.class.wip.interpret_StandardUnit('min'),    @(x) x./60,   @(y) 60.*y; ... % min
                WITio.class.wip.interpret_StandardUnit('s'),      @(x) x,       @(y) y; ... % s % DEFAULT
                WITio.class.wip.interpret_StandardUnit('ms'),     @(x) 1e3.*x,  @(y) 1e-3.*y; ... % ms
                WITio.class.wip.interpret_StandardUnit('µs'),     @(x) 1e6.*x,  @(y) 1e-6.*y; ... % µs
                WITio.class.wip.interpret_StandardUnit('ns'),     @(x) 1e9.*x,  @(y) 1e-9.*y; ... % ns
                WITio.class.wip.interpret_StandardUnit('ps'),     @(x) 1e12.*x, @(y) 1e-12.*y; ... % ps
                WITio.class.wip.interpret_StandardUnit('fs'),     @(x) 1e15.*x, @(y) 1e-15.*y}; % fs
            ValueUnit = Units{3,1}; % Default ValueUnit
        case Types{4}, % ZInterpretation
            if isstruct(I),
                x0 = I.TDZInterpretation.UnitName;
                % Try to recognize StandardUnit but allow non-StandardUnits
                x0 = WITio.class.wip.interpret_StandardUnit(x0);
            elseif isnan(I{2}), x0 = WITio.class.wip.ArbitraryUnit;
            else, x0 = WITio.class.wip.interpret_StandardUnit(I{2}); end
            Units = {x0, @(x) x, @(y) y};
            skipMatching = true;
            ValueUnit = WITio.class.wip.ArbitraryUnit; % Default ValueUnit
    end
    ...
    if ~isempty(Unit_old), % Interpret input if Unit_old is non-empty
        if ischar(Unit_old), % If given Unit_old is ValueUnit_old
            if ~skipMatching, % Skip only for ZInterpretation
                bw_find = strcmp(Units(:,1), WITio.class.wip.interpret_StandardUnit(Unit_old)); % First test if a Standard Unit
    ...
    if ~isempty(Unit_new), % Interpret output if Unit_new is non-empty
        if ischar(Unit_new), % If given Unit_new is ValueUnit_new
            if skipMatching, % Only for ZInterpretation
                ValueUnit = WITio.class.wip.interpret_StandardUnit(Unit_new);
            else, % Otherwise
                bw_find = strcmp(Units(:,1), WITio.class.wip.interpret_StandardUnit(Unit_new)); % First test if a Standard Unit
