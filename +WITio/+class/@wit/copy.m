...
function new = copy(obj),
    new = copy_children(obj, WITio.class.wit.empty); % Define as Root objects
    % Set Root properties
    for jj = 1:numel(obj),
    ...
    function new_children = copy_children(children, Parent),
        new_children = WITio.class.wit.empty; % Return empty if no obj given
        % Using constructor to automatically reset Root and Parent
%         new_children(numel(children)) = WITio.class.wit(); % Causes same-Id-bug when using Octave-compatible NextId-scheme!
        for ii = 1:numel(children),
            obj_ii = children(ii);
            new_ii = WITio.class.wit(); % Avoids same-Id-bug when using Octave-compatible NextId-scheme!
