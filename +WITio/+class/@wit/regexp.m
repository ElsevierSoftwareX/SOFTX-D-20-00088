...
function tags = regexp(obj, pattern, FirstOnly, LayersFurther, PrevFullNames),
    ...
    if isempty(obj), tags = WITio.class.wit.empty; return; end
    ...
    elseif LayersFurther < 1, % Special case: Limited search range
        ...
        tags = [reshape(obj(match), 1, []) regexp([subobj{:} WITio.class.wit.empty], pattern, FirstOnly, LayersFurther-1, reshape(PrevFullNames, 1, []))]; % Returns always a row vector
    end
end
