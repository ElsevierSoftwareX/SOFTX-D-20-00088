...
function bread(obj, buffer, N_bytes_max, swapEndianess, skip_Data_criteria_for_obj, error_criteria_for_obj, fun_progress_bar),
    ...
    if nargin < 4 || isempty(swapEndianess), swapEndianess = WITio.class.wit.swap_endianess(); end % By default: Binary with little endianess
    ...
    if nargin < 7, fun_progress_bar = @(x) WITio.class.wit.progress_bar(x, '-OnlyIncreasing'); end % By default: verbose progress bar in Command Window
    ...
    function isDone = bread_helper(obj),
        ...
        if ind_end > ind_max, % Abort, if the end is reached
            obj.ParentNow = WITio.class.wit.empty; % Do not touch obj.Parent.Data on deletion!
            return;
        end
        ...
        elseif obj_Type == 0, % Read the children
            children = WITio.class.wit.empty;
            while(ind_begin < obj_End), % Continue reading until DataEnd
                child = WITio.class.wit(); % Many times faster than WITio.class.wit(obj) due to redundant code
