...
% StandardUnit is a search string for WITio.wip.FullStandardUnits, where it is
...
function ValueUnit = interpret_StandardUnit(StandardUnit),
    ...
    bw_match = ~cellfun(@isempty, strfind(WITio.wip.FullStandardUnits, ['(' StandardUnit ')'])) | ...
        ~cellfun(@isempty, strfind(WITio.wip.FullStandardUnits, ['(' StandardUnit_known_specials ')']));
    if sum(bw_match) == 1, % Continue only if one match was found
        ValueUnit = WITio.wip.FullStandardUnits{bw_match}; % Modified output
    end
end
