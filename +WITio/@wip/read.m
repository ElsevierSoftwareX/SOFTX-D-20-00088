...
function [O_wid, O_wip, O_wid_HtmlNames] = read(varargin),
    ...
    O_wid = WITio.wid.empty;
    O_wip = WITio.wip.empty;
    ...
    for ii = 1:numel(files),
        ...
        if OnReadDecompress, % Read compressed
            ...
            O_wit_new = WITio.wit.read(files{ii}, LimitedRead);
            O_wit = WITio.wip.append(O_wit, O_wit_new);
        end
    end
    if ~ishandle(h), return; end % Abort if cancelled!
    ...
    function obj = OnReadDecompress_loop(obj, File),
        ...
        for jj = 1:numel(zip_datas),
            obj_new = WITio.wit.read(File, LimitedRead, [], [], '-CustomFun', @OnReadDecompress_helper, '-Silent');
            obj = WITio.wip.append(obj, obj_new);
        end
