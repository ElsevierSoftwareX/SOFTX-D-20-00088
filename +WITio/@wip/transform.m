...
function [ValueUnit, varargout] = transform(T, varargin),
    ...
    ValueUnit = WITio.wip.interpret_StandardUnit(SU); % Unit after transformation
    if strcmp(ValueUnit, SU), % Test whether StardardUnit was recognized or not
        ValueUnit = WITio.wip.ArbitraryUnit; % If not, then revert back to ArbitraryUnit
    end
    ...
    switch(T.Type),
        ...
        case 'TDSpaceTransformation',
            ...
            if isempty(ValueUnit), ValueUnit = WITio.wip.DefaultSpaceUnit; end
        case 'TDSpectralTransformation',
            fun = @SpectralTransformation;
            if isempty(ValueUnit), ValueUnit = WITio.wip.DefaultSpectralUnit; end
    ...
    [ValueUnit, varargout{1:numel(varargout)}] = WITio.wip.interpret(T_Data.TDTransformation.InterpretationID, [], ValueUnit, varargout{:});
