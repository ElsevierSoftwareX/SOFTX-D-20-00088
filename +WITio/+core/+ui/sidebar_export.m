...
%         set(h_sidebar, 'Position', [sidebar_Position(1)-sidebar_Position(3) sidebar_Position(2) 0 sidebar_Position(4)]); % Not backward compatible!
    ...
    [filename, pathname, filterindex] = uiputfile(formats, 'Export figure >> 600 DPI >> ...', WITio.core.pref.get('latest_folder', cd));
    
    % Export if filename is provided
    if pathname ~= 0,
        file = fullfile(pathname, filename);
        WITio.core.pref.set('latest_folder', pathname); % Remember permanently the latest folder
