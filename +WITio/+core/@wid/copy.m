...
function new = copy(obj),
    new = WITio.core.wid(size(obj)); % Return empty if no obj given
    for ii = 1:numel(obj),
        if ~obj(ii).isvalid, continue; end % Skip deleted
        new(ii).Project = obj(ii).Project; % Calls WITio.core.wid()-constructor!
        % Copy the tags
        obj_ii_Tag = obj(ii).Tag;
        if ~isempty(obj_ii_Tag),
            new_ii_Tag = struct;
            [new_ii_Tag.Root, Tags] = WITio.core.wip.append(obj_ii_Tag.Root, {[obj_ii_Tag.DataClassName obj_ii_Tag.Data]}); % Append the root (ENCLOSED BY {} TO AVOID TOUCHING THE LINKED IDS)
