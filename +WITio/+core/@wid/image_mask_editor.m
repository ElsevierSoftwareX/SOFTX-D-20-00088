...
function [new_obj, image_mask] = image_mask_editor(obj, image_mask),
    ...
    if ~isempty(obj) || nargin > 1,
        ...
        masking_mode = 0;
        show_mask();
        WITio.core.ui.sidebar_popup(Fig, 'Masking value:', {'False', 'True'}, @update_value, 1, false, [1 -1 1 1]);
        WITio.core.ui.sidebar_button(Fig, [], 'Clear mask', @clear, [1 1 1 -1]);
        WITio.core.ui.sidebar_button(Fig, [], 'Invert mask', @invert, [1 1 1 -1]);
        ...
        WITio.core.ui.sidebar_popup(Fig, 'Masking tool:', str_Popup, @update, 1, false, [1 -1 1 1]);
        
        AutoCloseInSeconds = WITio.core.pref.get('AutoCloseInSeconds', Inf);
        ...
        WITio.core.uiwait(Fig);
    end
    
    % Create new object if permitted
    if AutoCreateObj,
        new_obj = WITio.core.wid.new_Image(obj.Tag.Root); % This does not add newly created object to Project yet!
        ...
        if WITio.core.wip.get_Root_Version(obj) == 7,
