...
function h = plot_scalebar_helper(Ax, image_size, image_size_in_SU, image_SU, varargin),
    ...
    datas = WITio.fun.varargin_dashed_str.datas('Color', varargin, -1);
    color = [1 1 1]; % White by default
    if numel(datas) > 0, color = datas{1}; end

    % Check if Anchor was specified
    datas = WITio.fun.varargin_dashed_str.datas('Anchor', varargin, -1);
    ...
    datas = WITio.fun.varargin_dashed_str.datas('PositionRatio', varargin, -1);
    ...
    datas = WITio.fun.varargin_dashed_str.datas('Length', varargin, -1);
    length = []; % Auto length by default
    if numel(datas) > 0, length = datas{1}; end

    % Check if LengthUnit was specified
    datas = WITio.fun.varargin_dashed_str.datas('LengthUnit', varargin, -1);
    length_unit = [];
    if numel(datas) > 0, length_unit = datas{1}; end

    % Check if LengthFormat was specified
    datas = WITio.fun.varargin_dashed_str.datas('LengthFormat', varargin, -1);
    length_format = '%g';
    if numel(datas) > 0, length_format = datas{1}; end

    % Check if PatchVarargin was specified
    patch_varargin = WITio.fun.varargin_dashed_str.datas('PatchVarargin', varargin);

    % Check if TextVarargin was specified
    text_varargin = WITio.fun.varargin_dashed_str.datas('TextVarargin', varargin);

    % Check if notext was specified
    show_text = ~WITio.fun.varargin_dashed_str.exists('notext', varargin); % By default, show text

    % Interpret Length (if given) in correct units
    [~, length_in_SU] = WITio.core.wip.interpret('Space', image_SU, length_unit, length);
