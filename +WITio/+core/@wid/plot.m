    ...
    if ~isempty(Data),
        switch(obj.Type),
            case 'TDBitmap',
                ...
                if showSidebar && showCursor, WITio.core.ui.sidebar_cursor([], [], [], @transform_CP); end
            case 'TDGraph',
                ...
                switch(obj.SubType),
                    case 'Image', % Image
                        if showSidebar,
                            ...
                            fun_refresh = WITio.core.ui.sidebar_clim(Fig);
                            WITio.core.ui.sidebar_perceptually_uniform_colormaps(Fig);
                            WITio.core.ui.sidebar_sum_filter(Fig, @updateGraphImage, fun_refresh);
                            if showCursor,
                                if ~showPreview, WITio.core.ui.sidebar_cursor(Fig, [], [], @transform_CP);
                                else, WITio.core.ui.sidebar_cursor(Fig, @subPreview, [], @transform_CP); end
                            end
                        else,
                    ...
                    case 'Line', % Line
                        plotSpectrum(1, 1);
                        if showSidebar,
                            if showCursor, WITio.core.ui.sidebar_cursor(Fig); end
                            WITio.core.ui.sidebar_index_slider(Fig, Info.XSize, @(Ind) plotSpectrum(Ind, 1));
                            fun_auto = WITio.core.ui.sidebar_checkbox(Fig, 'Autoscale', 1);
                        end
                    case 'Point', % Point
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.core.ui.sidebar_cursor(Fig); end
                    case 'Array', % Array
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.core.ui.sidebar_cursor(Fig); end
                    ...
                    case 'Mask', % Mask
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.core.ui.sidebar_cursor(Fig); end        % CUSTOM TYPE
                    case 'Volume', % Volume (CUSTOM)
                        if showSidebar,
                            updateGraphVolume([-inf inf]);
                            WITio.core.ui.sidebar_perceptually_uniform_colormaps(Fig);
                            WITio.core.ui.sidebar_sum_filter(Fig, @updateGraphVolume);
                        else,
            ...
            case 'TDImage', % Image
                ...
                if showSidebar,
                    WITio.core.ui.sidebar_clim(Fig);
                    WITio.core.ui.sidebar_perceptually_uniform_colormaps(Fig);
                    if showCursor, WITio.core.ui.sidebar_cursor(Fig, [], [], @transform_CP); end
                end
            otherwise, % Text OR (un)formatted DataTree
                ...
                if showSidebar, % Show sidebar if requested
                    h_mainbar = WITio.core.ui.sidebar(Fig);
    ...
    function Text_cell = DataTree2TextCell(DT),
        ...
        for ll = 1:numel(values),
            ...
            if isstruct(value_ll), % CASE: A struct (== sub DataTree)
                ...
                if isa(value_ll, 'WITio.core.wid'), % CASE: A wid object
    ...
    function subPreview(CP),
        if ~isBusy, % Proceed only if NOT busy
            ...
            if isempty(Fig_sub) || ~ishandle(Fig_sub), 
                ...
                if showSidebar,
                    WITio.core.ui.sidebar_cursor(Fig_sub);
                    fun_auto = WITio.core.ui.sidebar_checkbox(Fig_sub, 'Autoscale', 1);
                end
            end
            % Show spectrum data
...
%             colormap(WITio.fun.lib.perceptually_uniform_colormap('inferno')); % Use inferno by default
    ...
    function h = plot_Spectrum(Fig, X, XUnit, Y, YUnit, fun, isAuto),
        ...
        if isempty(XUnit), XUnit = WITio.core.wip.ArbitraryUnit; end
        xlabel(Ax, XUnit);

        if isempty(YUnit), YUnit = WITio.core.wip.ArbitraryUnit; end
        ylabel(Ax, YUnit);
    end
end
