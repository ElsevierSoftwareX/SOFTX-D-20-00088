...
function [best_dist, best_obj] = binary_ind2obj(obj, ind),
    ...
    if isempty(obj), Roots = WITio.core.wit.empty;
    ...
    for jj = 1:numel(obj),
        for ii = 1:numel(pos),
            if best_dist(jj,ii) > 0,
                [best_dist(jj,ii), best_obj(jj,ii)] = binary_pos2obj_helper([best_obj(jj,ii).Children WITio.core.wit.empty], pos(ii), best_dist(jj,ii), best_obj(jj,ii));
            end
        end
    end
    function [best_dist, best_obj] = binary_pos2obj_helper(obj, pos, best_dist, best_obj), 
        ...
        if ~isempty(obj) && ~isempty(best_obj),
            [best_dist, best_obj] = binary_pos2obj_helper([best_obj.Children WITio.core.wit.empty], pos, best_dist, best_obj);
        end
    end
end
