...
function fread(obj, fid, N_bytes_max, swapEndianess, skip_Data_criteria_for_obj, error_criteria_for_obj, fun_progress_bar),
    ...
    if nargin < 4 || isempty(swapEndianess), swapEndianess = WITio.core.wit.swap_endianess(); end % By default: Binary with little endianess
    ...
    if nargin < 7, fun_progress_bar = @(x) WITio.core.wit.progress_bar(x, '-OnlyIncreasing'); end % By default: verbose progress bar in Command Window
    ...
    function isDone = fread_helper(obj),
        ...
        if feof(fid), % Abort, if file stream has reached the end
            obj.ParentNow = WITio.core.wit.empty; % Do not touch obj.Parent.Data on deletion!
            return;
        end
        ...
        elseif obj_Type == 0, % Read the children
            children = WITio.core.wit.empty;
            while(ftell(fid) < obj_End), % Continue reading until DataEnd
                child = WITio.core.wit(); % Many times faster than WITio.core.wit(obj) due to redundant code
