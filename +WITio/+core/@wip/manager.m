...
%         set(hcontainer, 'Units', 'normalized', 'Position', [0 0 1 1]); % Resize to fill the figure
        ...
        hcontainer.HTMLSource = fullfile(WITio.core.path.icons, 'uihtml_JList.html');
        drawnow;
        hcontainer.Data = S;
    ...
    if nargout > 0,
        WITio.core.uiwait(fig); % If output is expected, then wait until the manager is closed!
