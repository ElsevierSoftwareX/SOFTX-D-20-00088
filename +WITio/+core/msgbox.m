...
% EXAMPLE:
% h = WITio.core.msgbox('\bullet This is an{\bf example \color{magenta}dialog} box with max. {\fontname{Courier}200 px} wide text wrapping.', '-Icon', 'help', '-TextWrapping', 200, 'pixels');
function [h, rewrapped_message] = msgbox(message, varargin),
    ...
    if isempty(default_icondata) || isempty(default_iconcmap),
        [default_icondata, default_iconcmap] = imread(fullfile(WITio.core.path.toolbox, 'WITio.png'));
    end
