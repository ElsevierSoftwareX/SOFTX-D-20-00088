...
function tags = regexp(obj, pattern, FirstOnly, LayersFurther, PrevFullNames), %#ok
    ...
    else, FullNames = cellfun(@(x,y) [x '<' y], {obj.NameNow}, PrevFullNames, 'UniformOutput', false); end % Further sublevel FullName-calls are avoided
    ...
    elseif LayersFurther < 1, %#ok % Special case: Limited search range
        ...
        subobj = {obj.ChildrenNow}; % Collect the children
        ...
        tags = [reshape(obj(match), 1, []) regexp([subobj{:} WITio.obj.wit.empty], pattern, FirstOnly, LayersFurther-1, reshape(PrevFullNames, 1, []))]; % Returns always a row vector
    end
end
