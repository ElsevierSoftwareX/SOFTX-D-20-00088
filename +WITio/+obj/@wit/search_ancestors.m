...
function tags = search_ancestors(obj, varargin), %#ok
    ...
    if ~iscell(varargin{1}), match = strcmp({obj.NameNow}, varargin{1}); % Use strcmp
    else, match = ~cellfun(@isempty, regexp({obj.NameNow}, varargin{1}{1}, 'once')); end % Use regexp if enclosed in {}-brackets
    ...
    else, tags = search_ancestors([obj(match).ParentNow WITio.obj.wit.empty], varargin{2:end}); end % Continue search with the matched obj children
end
