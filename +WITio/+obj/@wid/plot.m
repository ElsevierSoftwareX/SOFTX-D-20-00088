    ...
    if ~isempty(Data),
        switch(obj.Type),
            case 'TDBitmap',
                ...
                if showSidebar && showCursor, WITio.tbx.ui.sidebar_cursor([], [], [], @transform_CP); end
            case 'TDGraph',
                ...
                switch(obj.SubType),
                    ...
                    case 'Line', % Line
                        plotSpectrum(1, 1);
                        if showSidebar,
                            if showCursor, WITio.tbx.ui.sidebar_cursor(Fig); end
                            WITio.tbx.ui.sidebar_index_slider(Fig, Info.XSize, @(Ind) plotSpectrum(Ind, 1));
                            fun_auto = WITio.tbx.ui.sidebar_checkbox(Fig, 'Autoscale', 1);
                        end
                    case 'Point', % Point
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.tbx.ui.sidebar_cursor(Fig); end
                    case 'Array', % Array
                        plotSpectrum(1, 1);
                        if showSidebar && showCursor, WITio.tbx.ui.sidebar_cursor(Fig); end
            ...
            case 'TDImage', % Image
                ...
                if showSidebar,
                    WITio.tbx.ui.sidebar_clim(Fig);
                    WITio.tbx.ui.sidebar_perceptually_uniform_colormaps(Fig);
                    if showCursor, WITio.tbx.ui.sidebar_cursor(Fig, [], [], @transform_CP); end
                end
    ...
    function Text_cell = DataTree2TextCell(DT),
        ...
        for ll = 1:numel(values),
            ...
            if isstruct(value_ll), % CASE: A struct (== sub DataTree)
                ...
                if isa(value_ll, 'WITio.obj.wid'), % CASE: A wid object
...
%             end
    ...
    function updateGraphVolume(filter_range),
        set(0, 'CurrentFigure', Fig);
        Data_range = WITio.obj.wid.crop_Graph_with_bg_helper(Data, Info.Graph, filter_range);
    ...
    function subPreview(CP),
        if ~isBusy, % Proceed only if NOT busy
            ...
            if isempty(Fig_sub) || ~ishandle(Fig_sub), 
                ...
                if showSidebar,
                    WITio.tbx.ui.sidebar_cursor(Fig_sub);
                    fun_auto = WITio.tbx.ui.sidebar_checkbox(Fig_sub, 'Autoscale', 1);
                end
            end
            % Show spectrum data
...
%             colormap(WITio.fun.lib.perceptually_uniform_colormap('inferno')); % Use inferno by default
    ...
    function h = plot_Spectrum(Fig, X, XUnit, Y, YUnit, fun, isAuto),
        ...
        if isempty(XUnit), XUnit = WITio.obj.wip.ArbitraryUnit; end
        xlabel(Ax, XUnit);

        if isempty(YUnit), YUnit = WITio.obj.wip.ArbitraryUnit; end
        ylabel(Ax, YUnit);
    end
end
