...
function [Data_range, Graph_range, Data_range_bg, range] = crop_Graph_with_bg_helper(Data, Graph, range, bg_avg_lower, bg_avg_upper),
    ...
    Data_mean_start = WITio.fun.indep.mynanmean(Data_start, 3);
    Graph_mean_start = WITio.fun.indep.mynanmean(Graph_start, 3);
    
    % Calculate upper background averaging
    [Data_end, Graph_end] = quick_indexing(idx_upper+1, idx_upper+bg_avg_upper);
    Data_mean_end = WITio.fun.indep.mynanmean(Data_end, 3);
    Graph_mean_end = WITio.fun.indep.mynanmean(Graph_end, 3);
