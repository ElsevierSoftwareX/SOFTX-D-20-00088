...
function [ValueUnit, varargout] = transform(T, varargin),
    ...
    ValueUnit = WITio.obj.wip.interpret_StandardUnit(SU); % Unit after transformation
    if strcmp(ValueUnit, SU), % Test whether StardardUnit was recognized or not
        ValueUnit = WITio.obj.wip.ArbitraryUnit; % If not, then revert back to ArbitraryUnit
    end
    ...
    switch(T.Type),
        ...
        case 'TDSpaceTransformation',
            ...
            if isempty(ValueUnit), ValueUnit = WITio.obj.wip.DefaultSpaceUnit; end
        case 'TDSpectralTransformation',
            fun = @SpectralTransformation;
            if isempty(ValueUnit), ValueUnit = WITio.obj.wip.DefaultSpectralUnit; end
    ...
    [ValueUnit, varargout{1:numel(varargout)}] = WITio.obj.wip.interpret(T_Data.TDTransformation.InterpretationID, [], ValueUnit, varargout{:});
