...
function [O_wit, varargout] = append(varargin),
    ...
    % Test input first
    ind_notwit = find(~cellfun(@(x) isa(x, 'WITio.obj.wit'), varargin), 1);
    ...
    DataOrDataClassNames = WITio.obj.wit.empty;
    ViewerOrViewerClassNames = WITio.obj.wit.empty;
    ...
    for ii = 1:numel(varargin),
        ...
        Data_Pairs = WITio.obj.wip.get_Data_DataClassName_pairs(O_wit_ii);
        Viewer_Pairs = WITio.obj.wip.get_Viewer_ViewerClassName_pairs(O_wit_ii);
        ...
        if ~isempty(Data_Pairs),
            ...
            for jj = 1:numel(Tags_with_ID),
                ...
                if isa(Data_jj, 'WITio.obj.wit'), % SPECIAL CASE: ID list
            ...
            ID_max = 0;
            for jj = 1:numel(Tags_with_ID),
                if isa(Tags_with_ID(jj).Data, 'WITio.obj.wit'), % SPECIAL CASE: ID list
                    ID_list = Tags_with_ID(jj).search_children('Data');
