...
function [ValueUnit, varargout] = interpret_forced(obj, I, Unit_new, Unit_old, varargin),
    ...
    if isa(I, 'WITio.obj.wid'), Type = I.Type; % Get its Type
    elseif iscell(I) && numel(I) == 2, Type = I{1};
    elseif ischar(I), Type = I; end

    % Interpret input
    [ValueUnit, varargout{1:nargout-1}] = WITio.obj.wip.interpret(I, Unit_new, Unit_old, varargout{:});
    % Override units
    if ~isempty(obj),
        ForceUnit = [];
        switch(Type),
        ...
        if ~isempty(ForceUnit),
            [ValueUnit, varargout{1:nargout-1}] = WITio.obj.wip.interpret(I, ForceUnit, ValueUnit, varargout{:});
        end
    end
end
