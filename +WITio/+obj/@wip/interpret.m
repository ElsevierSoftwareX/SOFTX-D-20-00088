...
function [ValueUnit, varargout] = interpret(I, Unit_new, Unit_old, varargin),
    ...
    switch(Type),
        case Types{1}, % SpaceInterpretation
            Units = {WITio.obj.wip.interpret_StandardUnit('m'), @(x) 1e-6.*x, @(y) 1e6.*y; ... % m
                WITio.obj.wip.interpret_StandardUnit('mm'),     @(x) 1e-3.*x, @(y) 1e3.*y; ... % mm
                WITio.obj.wip.interpret_StandardUnit('µm'),     @(x) x,       @(y) y; ... % µm % DEFAULT
                WITio.obj.wip.interpret_StandardUnit('nm'),     @(x) 1e3.*x,  @(y) 1e-3.*y; ... % nm
                WITio.obj.wip.interpret_StandardUnit('Å'),      @(x) 1e4.*x,  @(y) 1e-4.*y; ... % Å
                WITio.obj.wip.interpret_StandardUnit('pm'),     @(x) 1e6.*x,  @(y) 1e-6.*y}; % pm
        ...
        case Types{2}, % SpectralInterpretation
            ...
            Units = {WITio.obj.wip.interpret_StandardUnit('nm'),   @(x) x,                           @(y) y; ... % nm % DEFAULT
                WITio.obj.wip.interpret_StandardUnit('µm'),        @(x) 1e-3.*x,                     @(y) 1e3.*y; ... % µm
                WITio.obj.wip.interpret_StandardUnit('1/cm'),      @(x) 1e7./x,                      @(y) 1e7./y; ... % 1/cm
                WITio.obj.wip.interpret_StandardUnit('rel. 1/cm'), @(x) 1e7.*(1./x0-1./x),           @(y) 1./(1./x0-1e-7.*y); ... % rel. 1/cm
                WITio.obj.wip.interpret_StandardUnit('eV'),        @(x) 1.23984193e3./x,             @(y) 1.23984193e3./y; ... % eV
                WITio.obj.wip.interpret_StandardUnit('meV'),       @(x) 1.23984193e6./x,             @(y) 1.23984193e6./y; ... % meV
                WITio.obj.wip.interpret_StandardUnit('rel. eV'),   @(x) -1.23984193e3.*(1./x0-1./x), @(y) 1./(1./x0+y./1.23984193e3); ... % rel. eV
                WITio.obj.wip.interpret_StandardUnit('rel. meV'),  @(x) -1.23984193e6.*(1./x0-1./x), @(y) 1./(1./x0+y./1.23984193e6)}; % rel. meV
            ValueUnit = Units{1,1}; % Default ValueUnit
        case Types{3}, % TimeInterpretation
            Units = {WITio.obj.wip.interpret_StandardUnit('h'), @(x) x./3600, @(y) 3600.*y; ... % h
                WITio.obj.wip.interpret_StandardUnit('min'),    @(x) x./60,   @(y) 60.*y; ... % min
                WITio.obj.wip.interpret_StandardUnit('s'),      @(x) x,       @(y) y; ... % s % DEFAULT
                WITio.obj.wip.interpret_StandardUnit('ms'),     @(x) 1e3.*x,  @(y) 1e-3.*y; ... % ms
                WITio.obj.wip.interpret_StandardUnit('µs'),     @(x) 1e6.*x,  @(y) 1e-6.*y; ... % µs
                WITio.obj.wip.interpret_StandardUnit('ns'),     @(x) 1e9.*x,  @(y) 1e-9.*y; ... % ns
                WITio.obj.wip.interpret_StandardUnit('ps'),     @(x) 1e12.*x, @(y) 1e-12.*y; ... % ps
                WITio.obj.wip.interpret_StandardUnit('fs'),     @(x) 1e15.*x, @(y) 1e-15.*y}; % fs
            ValueUnit = Units{3,1}; % Default ValueUnit
