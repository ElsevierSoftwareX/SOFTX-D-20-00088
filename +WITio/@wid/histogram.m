...
%         Data_upper_quantile = interp1([0 1], [cmin cmax], upper_quantile, 'linear'); % Approximate upper quantile
        ...
        Bin_Centers = reshape((Bin_Edges(1:end-1)+Bin_Edges(2:end))./2, [], 1); % Get Bin Centers because WITio.wid.plot uses built-in bar-function
    ...
    if AutoCreateObj,
        new_obj = WITio.wid.new_Graph(obj.Tag.Root); % This does not add newly created object to Project yet!
        ...
        new_TLUT = WITio.wid.new_Transformation_LUT(obj.Tag.Root, numel(Bin_Centers)); % This does not add newly created object to Project yet!
        ...
        new_ILUT = WITio.wid.new_Interpretation_Z(obj.Tag.Root); % This does not add newly created object to Project yet!
