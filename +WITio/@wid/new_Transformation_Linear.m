...
function obj = new_Transformation_Linear(O_wit),
    ...
    Tag_Extra = WITio.wit('TDLinearTransformation', [ ...
        WITio.wit('Version', int32(0)) ...
        WITio.wit('ModelOrigin_D', double(0)) ... % (NOT IN ALL LEGACY VERSIONS)
        WITio.wit('WorldOrigin_D', double(0)) ... % (NOT IN ALL LEGACY VERSIONS) % 1 for null-transformation in MATLAB
        WITio.wit('Scale_D', double(1)) ... % (NOT IN ALL LEGACY VERSIONS)
        WITio.wit('ModelOrigin', single(0)) ...
        WITio.wit('WorldOrigin', single(0)) ... % 1 for null-transformation in MATLAB
        WITio.wit('Scale', single(1)) ...
        ]);
    
    Tag_DataClassName = WITio.wit('DataClassName 0', 'TDLinearTransformation');
