...
function [new_obj, Graph, Data, W, D] = spectral_stitch(obj, varargin),
    ...
    bw_valid_wid = cellfun(@(x) isa(x, 'WITio.wid'), varargin); % Test if wid
    ...
    [Graph, Data, W, D] = WITio.wid.spectral_stitch_helper(Graphs_nm, Datas, isdebug);
    ...
    if AutoCreateObj,
        ...
        new_SpectralT = WITio.wid.new_Transformation_LUT(obj.Tag.Root, numel(Graph)); % This does not add newly created object to Project yet!
