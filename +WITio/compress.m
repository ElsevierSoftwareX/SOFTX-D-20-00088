...
function compress(file, files, datas, varargin),
    ...
    parsed = WITio.parse.varargin_dashed_str_datas('Compressor', varargin, -1);
    ...
    elseif strcmpi('.zst', Compressor),
        ...
        if isempty(zst_library), % Load java library only once per session
            ...
            zst_library = fullfile(WITio.path.package, compressor_library{:});
    ...
    parsed = WITio.parse.varargin_dashed_str_datas('CompressionLevel', varargin, -1);
