...
function F = fun_lineshape_voigtian(P, X),
    ...
    nominator = real(WITio.lib.fadf.fadf(Z)); % Voigtian lineshape profile
    divisor = real(WITio.lib.fadf.fadf(z)); % Maximum of the Voigtian lineshape profile % = exp(imag(z_max).^2).*erfc(imag(z_max));
...
%     nominator_2 = real(-WITio.lib.fadf.fadf(-Z)); % Shoots quickly to infinity when Z > 26
%     
...
%     % 
%     % divisor = real(WITio.lib.fadf.fadf(z)) ...
