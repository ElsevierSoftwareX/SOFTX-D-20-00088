...
function [ind, isAnyAtClassMax, isAtClassMax] = generic_sub2ind(arraySize, varargin),
    ...
    B_dashed = strncmp(varargin, '-', 1); % Call to WITio.fun.varargin_dashed_str.any_exists(varargin) avoided for slight speed-up
    if any(B_dashed), % Parse ONLY IF exists (for speed-up!)
        ...
        if N_dashed > 1 || (N_dashed == 1 && B_continue),
            [doTruncate, datas] = WITio.fun.varargin_dashed_str.exists_and_datas('truncate', strs_dashed, -1);
            if doTruncate && numel(datas) > 0, doTruncate = logical(datas{1}); end

            [doReplace, datas] = WITio.fun.varargin_dashed_str.exists_and_datas('replace', strs_dashed, -1);
            if doReplace && numel(datas) > 0, doReplace = logical(datas{1}); end

            datas = WITio.fun.varargin_dashed_str.datas('value', strs_dashed, -1);
            value = NaN; % Default replacement value
            if numel(datas) > 0, value = datas{1}; end

            [doMirror, datas] = WITio.fun.varargin_dashed_str.exists_and_datas('mirror', strs_dashed, -1);
            if doMirror && numel(datas) > 0, doMirror = logical(datas{1}); end

            [doCirculate, datas] = WITio.fun.varargin_dashed_str.exists_and_datas('circulate', strs_dashed, -1);
