...
%-------------------------------------------------------------------------%
[O_wid, O_wip, O_wit] = WITio.read(file, '-all', '-SpectralUnit', 'rel. 1/cm'); % Load all the file plottable content and force SpectralUnit to Raman shift
...
figure; O_Point.plot('-compare', O_Point_0(end), O_Point_D(end), O_Point_G(end), O_Point_2D(end)); WITio.tbx.ifnodesktop(); % Show fitting results % Image<TDGraph with sidebar
