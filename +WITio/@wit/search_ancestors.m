...
function tags = search_ancestors(obj, varargin),
    ...
    if ~any(match), tags = WITio.wit.empty; return; end % Return empty if no matches
    if numel(varargin) == 1, tags = obj(match); % Return matches if no more criteria found
    else, tags = search_ancestors([obj(match).Parent WITio.wit.empty], varargin{2:end}); end % Continue search with the matched obj children
end
