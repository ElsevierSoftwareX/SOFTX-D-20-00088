...
function fwrite(obj, fid, swapEndianess, fun_progress_bar),
    if nargin < 3 || isempty(swapEndianess), swapEndianess = WITio.wit.swap_endianess(); end % By default: Binary with little endianess
    if nargin < 4, fun_progress_bar = @(x) WITio.wit.progress_bar(x, '-OnlyIncreasing'); end % By default: verbose progress bar in Command Window
