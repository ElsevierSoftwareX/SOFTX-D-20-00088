...
%     predefined_isVisible = format(:,2); % IGNORED
    ...
    children = reshape([parent.Children WITio.wit.empty], [], 1); % Force column
    ...
    for ii = 1:numel(predefined_names),
        ...
        if ind_to_C_from_P(ii),
            ...
            child = WITio.wit(parent, name); % Append new child to the WIT-tree
        end
        ...
        if isempty(value) || size(value, 2) == 3, % CASE: empty OR YES subformat
            ...
            if ind_to_I_from_P(ii), % CASE: YES found in-struct field
                WITio.wit.DataTree_set(child, in_values{ind_to_I_from_P(ii)}, value);
            else, % CASE: NO found in-struct field
                WITio.wit.DataTree_set(child, struct(), value);
            end
    ...
    for ii = 1:numel(in_fields),
        ...
        if ind_to_C_from_I(ii),
            ...
            child = WITio.wit(parent, field); % Non-linked in-struct field string is name
        end

        if isstruct(value),
            WITio.wit.DataTree_set(child, value, {});
        else,
            delete(child.Children); % Destroy the underlying wit-tree
            child.Data = value;
        end
